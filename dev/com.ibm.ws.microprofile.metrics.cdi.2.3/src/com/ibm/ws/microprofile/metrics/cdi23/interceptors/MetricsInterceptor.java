...
import io.astefanutti.metrics.cdi23.MetricsExtension;
...
public class MetricsInterceptor {
    ...
    private <E extends Member & AnnotatedElement> void registerMetrics(Class<?> bean, E element) throws Exception {
        ...
        if (true) {
            System.out.println("MetricsInterceptor 2.3: About to throw an exception");
        ...
        if (simplyTimed.isPresent()) {
            registry.simpleTimer(simplyTimed.metadata(), Utils.tagsToTags(simplyTimed.tags()));
            MetricID mid = new MetricID(simplyTimed.metadata().getName(), Utils.tagsToTags(simplyTimed.tags()));
            extension.addMetricID(element, simplyTimed.metricAnnotation(), mid);
        }
    }
